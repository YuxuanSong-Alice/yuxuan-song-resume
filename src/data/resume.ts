export const resumeData = {
  zh: {
    name: "宋雨萱",
    title: "数字化增长 | 策略产品运营 | 用户全生命周期运营",
    contact: {
      email: "yuxuansong0930@163.com",
      phone: "13488976132",
      wechat: "SyuxuanS0930",
      dob: "1999.09.30"
    },
    education: [
      {
        school: "哥伦比亚大学",
        degree: "企业风险管理 硕士",
        period: "09/2021-02/2023",
        gpa: "3.83/4.0"
      },
      {
        school: "罗格斯大学",
        degree: "供应链管理 本科",
        period: "09/2017-06/2021",
        gpa: "3.55/4.0（Cum Laude 优等毕业生荣誉）"
      }
    ],
    experience: [
      {
        company: "拼多多",
        location: "上海",
        position: "数字化增长 | 策略产品运营 | 用户全生命周期运营",
        period: "04/2023-04/2026（3年）",
        description:
          "3年全球化用户增长、策略产品运营、数据驱动闭环经验，擅长用户生命周期管理、大模型策略优化、全域增长落地。"
      }
    ],
    projects: [
      {
        title: "海外市场用户增长与精准广告投放",
        description:
          "主导美国、瑞士、捷克、斯洛伐克、西班牙等多国 Google/Facebook/TikTok 全渠道广告投放，覆盖预算分配、智能出价、人群定向、多语言素材制作及落地页转化全流程优化。搭建前端曝光-点击-激活-支付全链路数据闭环，基于区域用户行为差异制定差异化投放策略，显著提升 ROI。",
        results:
          "负责捷克、斯洛伐克市场期间，实现区域预估 GMV 增长 200%+，平台下载率提升 70%+，快速完成新市场冷启动与用户心智渗透。"
      },
      {
        title: "大模型风险识别系统优化（核心战略项目）",
        description:
          "针对传统风险识别模型准确率不足、误判率偏高引发大量客诉与舆情风险的业务痛点，联动算法团队明确优化目标，搭建行业级标准化标注规范。从任务定义、角色边界、术语规范、输出标准四大维度，构建结构化大模型 Prompt 训练体系，主导业务侧识别效果评估，沉淀可规模化复用的模型迭代数据闭环方法论。",
        results:
          "模型风险行为识别准确率由 17% 提升至 35%，误判率下降 40%，用户因误判产生的负面反馈减少 60% 以上。"
      },
      {
        title: "用户合规服务柔性机制设计与落地",
        description:
          "作为用户关系体验优化项目负责人，搭建分级分类、场景化的首次合规提醒与柔性处理体系，依据风险等级制定差异化处置规则，完成全链路校验逻辑梳理与全流程交互方案输出。协同产品、研发、算法团队完成项目开发与灰度上线，建立「规则宣导—执行落地—效果反馈」的运营机制。",
        results:
          "日均覆盖 2.5 万+ 业务订单，用户合规相关不满反馈占比下降 14%，在强化规则认知的同时显著降低用户抵触情绪，二次风险行为发生率下降 8%。"
      },
      {
        title: "全链路用户体验优化与关系运营升级",
        description:
          "梳理用户服务全生命周期核心情绪触点，搭建协商前置调解、判后主动关怀、异议快速响应的全链路用户关系干预体系。聚焦平台 TOP5 高冲突场景开展专项治理，推动三大核心问题季度环比显著改善：事前沟通不及时问题下降 10%；事后履约纠纷问题下降 4%；服务合规争议问题下降 19%。",
        results:
          "整体服务求助率从 18.7% 降至 12.5%（季度环比），平台用户满意度提升 12%。"
      },
      {
        title: "内部运营流程数字化与 AI 效率提升",
        description:
          "牵头风险订单初筛核实流程线上化改造，落地大模型应用实现非结构化数据自动解析、风险智能分级与工单自动分配，推动运营流程数字化升级。",
        results:
          "团队日均订单复核效率由 20 单提升至 45 单，外包人工重复劳动量减少 40%，实现数据驱动效率提升与运营成本优化，强化全链路运营闭环能力。"
      }
    ],
    skills: {
      languages: "中文（母语），英文（流利）",
      tools:
        "Google Ads Manager, Meta Business Suite, TikTok Ads, Gemini, NotebookLM, Prompt Engineering, Notion",
      interests: "钢琴，舞蹈，健身，阅读"
    }
  },
  en: {
    name: "Yuxuan Song",
    title: "Global User Growth | Strategy Product Manager | Strategy & Product Operation",
    contact: {
      email: "yuxuansong0930@163.com",
      phone: "13488976132",
      wechat: "SyuxuanS0930",
      dob: "1999.09.30"
    },
    education: [
      {
        school: "Columbia University",
        degree: "M.S. in Enterprise Risk Management",
        period: "09/2021-02/2023",
        gpa: "3.83/4.0"
      },
      {
        school: "Rutgers University",
        degree: "B.S. in Supply Chain Management",
        period: "09/2017-06/2021",
        gpa: "3.55/4.0（Cum Laude）"
      }
    ],
    experience: [
      {
        company: "Pinduoduo",
        location: "Shanghai",
        position: "Global User Growth | Strategy Product Manager | Strategy & Product Operation",
        period: "04/2023-04/2026（3yrs）",
        description:
          "3+ years of global user growth, strategy product operations, and data-driven closed-loop execution. Strong in user lifecycle management, LLM strategy optimization, and full-funnel growth."
      }
    ],
    projects: [
      {
        title: "Global User Growth & Performance Marketing",
        description:
          "Managed full-funnel advertising across Google, Facebook, and TikTok for markets including the US, Switzerland, Spain, Czech Republic, and Slovakia. Optimized budget allocation, smart bidding, and localized creative assets based on regional user behavior insights.",
        results:
          "Achieved 200%+ GMV growth and a 70%+ increase in app downloads in the Czech and Slovak markets, successfully establishing brand penetration in new territories."
      },
      {
        title: "LLM-Driven Abuse Detection System Optimization (Strategic Initiative)",
        description:
          "Identified pain points regarding low model accuracy and high false-positive rates that led to merchant escalations. Partnered with Algorithm teams to develop an industry-leading annotation standard and a structured Prompt Engineering framework (Context, Role, Definition, and Output). Led the annotation of 10,000+ samples and established a repeatable model iteration methodology.",
        results:
          "Increased abuse detection rates from 17% to 35%, reduced false positives by 40%, and decreased negative merchant feedback by over 60%."
      },
      {
        title: 'Merchant Service Compliance & "First-Time Violation Waiver" Mechanism',
        description:
          "Headed a pilot program to alleviate merchant friction by designing a tiered \"First-Time Violation Exemption\" system based on risk levels and business scenarios. Coordinated with Product, R&D, and Legal teams to execute a phased rollout and established a \"Policy-Execution-Feedback\" operational loop.",
        results:
          "Covered 25k+ daily orders, reducing penalty-related dissatisfaction by 14% and secondary violation rates by 8% while reinforcing policy awareness."
      },
      {
        title: "Merchant Sentiment Optimization & Experience Enhancement",
        description:
          "Mapped core emotional touchpoints across the after-sales lifecycle and built an intervention system featuring pre-settlement mediation and rapid dispute response. Spearheaded a task force to optimize the Top 5 high-conflict scenarios, driving significant quarterly improvements: Pre-judgment Communication Gaps improved by 10%; Post-judgment Fulfillment Disputes reduced by 4%; Service Attitude Penalty Controversies reduced by 19%.",
        results:
          "Reduced overall after-sales help-seeking rates from 18.7% to 12.5% and boosted platform merchant satisfaction by 12 percentage points."
      },
      {
        title: "Operational Digitalization & AI Efficiency Enhancement",
        description:
          "Led the digitalization of manual review processes by integrating LLM capabilities for unstructured data parsing, automated risk grading, and intelligent ticket routing.",
        results:
          "Increased team review efficiency from 20 to 35 orders/day, reduced manual repetition by 40%, and shortened the total operational cycle by 30%."
      }
    ],
    skills: {
      languages: "Chinese (Native), English (Fluent)",
      tools:
        "Google Ads Manager, Meta Business Suite, TikTok Ads, Gemini, NotebookLM, Prompt Engineering, Notion",
      interests: "Piano, Dance, Fitness, Reading"
    }
  }
};
